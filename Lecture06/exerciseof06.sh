...
rm -f *.exercise.out
inputfile="blastoutputformat7.out"
...
# Remember, bash CAN'T deal properly with non-integers, so make it an integer		
	bitscore=$(printf "%.0f\n" ${bitscore})
		...
		echo -e "${dataline}\t${alignment_length}\t${pc_identity}" >> al_leng_peID.exercise.out
		...
		>> mismatchespercent.exercise.out"

		scorebin=1
		if [ ${bitscore} -gt ${group3cut} ]; then
			scorebin=4
		fi
		if [ ${bitscore} -le ${group3cut} ] && [ ${bitscore} -gt ${group1cut} ]; then
			scorebin=3
		fi
...
# use a case satatement to send the output to the relevant file 
		scoregroupdetails=$(echo -e "${dataline}\t${Q_acc}\t${bitscore}")
		case $scorebin in 
			4)
...
# END block EQUIVALENT
	...
	fi # was not a commend line in the blast data
done < ${inputfile}
